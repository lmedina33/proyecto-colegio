...
class DAOGestionAlumnos extends Conexion{
    ...
    function admin_insertar_alumno($codigo,$nombres,$apellido_p,$apellido_m,$password,$grado,$nivel,$seccion){
        ...
        if($cn!="no_conexion"){
        	...
        	$sql="insert into $this->nombre_tabla_alumnos (codigo,nombres,apellido_paterno,apellido_materno,edad,password,disponible,id_padre,grado,nivel,seccion) values ('$codigo','$nombres','$apellido_p','$apellido_m',0,'$password',1,' ','$grado','$nivel','$seccion')";
			    $rs = mysql_query($sql,$cn);
			echo $sql;
			mysql_close($cn);			 
			return "mysql_si";
	...
	function admin_get_alumnos($nivel,$grado,$seccion){
        ...
        if($cn!="no_conexion"){
        	...
        	$sql="select * from $this->nombre_tabla_alumnos where nivel='$nivel' and grado=$grado and seccion='$seccion'";	
			   $rs = mysql_query($sql,$cn);
			...
			if($consulta){
				...
				$respuesta.=$c->codigo."{".$c->nombres."{".$c->apellido_paterno."{".$c->apellido_materno."{".$c->password."{".$c->id_padre."{".$c->disponible."{";
