...
function fun_resalta_fila_tabla(arg_fila_resaltar,arg_tabla,arg_color_fondo,arg_color_letra){ // los parámetros sirven para especificar la fila que se va a resaltar y de que color se va a resaltar el fondo y la letra. normalmente se usa blue y white para resaltar la fila.
		...
		$(arg_tabla+" #"+arg_fila_resaltar+" td").css("background-color",arg_color_letra);	
		...
		$(div+" #"+arg_fila_resaltar).css("color",arg_color_fondo);
		$(div+" #"+arg_fila_resaltar).attr("title",'Fila seleccionada');
...
//arg_tabla debe conener la ubicacion de la tabla ya sea por medio de varios div, pero debe terminar en table ejemplo: "#div_area #pop_up_x table"
	...
	$(arg_tabla+" tr").each(function(){
		...
		if($(this).children().css("background-color")==arg_color_fondo || $(this).children().css("background-color")==arg_color_fondo_rgb ){
